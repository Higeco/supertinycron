...
time_t cron_mktime(struct tm* tm) {
    return g_time_funcs.to_epoch(g_time_funcs.context, tm);
}

struct tm* cron_time(time_t* date, struct tm* out) {
    return g_time_funcs.from_epoch(g_time_funcs.context, date, out);
}

void cron_set_time_funcs(cron_time_funcs funcs) {
    g_time_funcs = funcs;
    if (!g_time_funcs.to_epoch) g_time_funcs.to_epoch = default_cron_mktime;
    if (!g_time_funcs.from_epoch) g_time_funcs.from_epoch = default_cron_time;
}
